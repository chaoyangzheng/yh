...
public class HomePageController {
     ...
     */
    @ApiOperation(value = "查询所有总类别", notes = "查询所有总类别的list集合，用于展示首页顶部菜单选项，不包括全部，推荐，vip专区")
     ...
     */
    @ApiOperation(value = "查询所有课程类型", notes = "查询所有课程类型的list集合，用于展示到首页|推荐页面的轮播图下方菜单列表，不包括名师课堂")
     ...
     */
    @ApiOperation(value = "随机查询艺听就懂课程", notes = "随机查询1个基础公开课，用于展示到首页|推荐页面的艺听就懂")
     ...
     */
    @ApiOperation(value = "查询今日推荐课程", notes = "查询今日最热门的1个基础公开课，用于展示到首页|推荐页面的今日推荐")
    @PostMapping("/todayRecommendVideoCourse.do")
    public JsonResult todayRecommendVideoCourse() {
        try {
            VideoCourse videoCourse = videoCourseService.findHottestVideoCourse();
            return new JsonResult("0", videoCourse);
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * @author SHIGUANGYI
     * @date 2019/10/3
     * 按综合排序查询所有课程，用于展示到首页|全部页面的综合
     */
    @ApiOperation(value = "按综合排序查询所有课程", notes = "按综合排序查询所有课程，用于展示到首页|全部页面的综合")
    @ApiImplicitParams({
    ...
    public JsonResult comprehensiveVideoCourse(Integer pageNum, Integer pageSize) {
        try {
            ...
            List<VideoCourse> videoCourseList = videoCourseService.findComprehensiveVideoCourse(pageNum, pageSize);
     ...
     * @author SHIGUANGYI
     * @date 2019/10/3
     * 按热门排序查询所有课程，用于展示到首页|全部页面的热门
     */
    @ApiOperation(value = "按热门排序查询所有课程", notes = "按热门排序查询所有课程，用于展示到首页|全部页面的热门")
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "query", name = "pageNum", value = "当前页码", dataType = "Integer", defaultValue = "1"),
            @ApiImplicitParam(paramType = "query", name = "pageSize", value = "每页条数", dataType = "Integer", defaultValue = "10")
    })
    @PostMapping("/hottestVideoCourse.do")
    public JsonResult hottestVideoCourse(Integer pageNum, Integer pageSize) {
        try {
            if (null == pageNum || "".equals(pageNum)) {
                pageNum = 1;
            }
            ...
            List<VideoCourse> videoCourseList = videoCourseService.findHottestVideoCourse(pageNum, pageSize);
            return new JsonResult("0", videoCourseList);
        } catch (Exception e) {
            e.printStackTrace();
            return new JsonResult("1", "查询失败" + e.getMessage());
